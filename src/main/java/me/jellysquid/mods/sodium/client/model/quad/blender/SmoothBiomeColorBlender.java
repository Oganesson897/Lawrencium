...
public class SmoothBiomeColorBlender implements BiomeColorBlender {
    ...
    public int[] getColors(IBlockColor colorizer, IBlockAccess world, IBlockState state, BlockPos origin,
        ...
        boolean aligned = ModelQuadFlags.contains(quad.lawrencium$getFlags(), ModelQuadFlags.IS_ALIGNED);
    ...
    private int getVertexColor(IBlockColor colorizer, IBlockAccess world, IBlockState state, BlockPos origin,
        ...
        final int x = origin.getX() + (int) quad.lawrencium$getX(vertexIdx);
        final int z = origin.getZ() + (int) quad.lawrencium$getZ(vertexIdx);

        final int color = this.getBlockColor(colorizer, world, state, origin, x, z, quad.lawrencium$getColorIndex());
    ...
    private int getInterpolatedVertexColor(IBlockColor colorizer, IBlockAccess world, IBlockState state,
        ...
        final float x = quad.lawrencium$getX(vertexIdx);
        final float z = quad.lawrencium$getZ(vertexIdx);
        ...
        final int c1 = this.getBlockColor(colorizer, world, state, origin, originX, originZ, quad.lawrencium$getColorIndex());
        final int c2 = this.getBlockColor(colorizer, world, state, origin, originX, originZ + 1, quad.lawrencium$getColorIndex());
        final int c3 = this.getBlockColor(colorizer, world, state, origin, originX + 1, originZ, quad.lawrencium$getColorIndex());
        final int c4 = this.getBlockColor(colorizer, world, state, origin, originX + 1, originZ + 1, quad.lawrencium$getColorIndex());
