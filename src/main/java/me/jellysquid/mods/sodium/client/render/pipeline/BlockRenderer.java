...
public class BlockRenderer {
    ...
    private void renderQuadList(IBlockAccess world, IBlockState state, BlockPos pos, LightPipeline lighter, Vec3d offset,
        ...
        for (int i = 0, quadsSize = quads.size(); i < quadsSize; i++) {
            ...
            if (quad.hasTintIndex() && colorizer == null) {
                if (this.blockColors.lawrencium$hasColorProvider(state)) {
                    colorizer = this.blockColors.lawrencium$getColorProvider(state);
                }
            }
    ...
    private void renderQuad(IBlockAccess world, IBlockState state, BlockPos pos, ModelVertexSink sink, Vec3d offset,
        ...
        for (int dstIndex = 0; dstIndex < 4; dstIndex++) {
            ...
            float x = src.lawrencium$getX(srcIndex) + (float) offset.x;
            float y = src.lawrencium$getY(srcIndex) + (float) offset.y;
            float z = src.lawrencium$getZ(srcIndex) + (float) offset.z;

            int color = ColorABGR.mul(colors != null ? colors[srcIndex] : src.lawrencium$getColor(srcIndex), light.br[srcIndex]);

            float u = src.lawrencium$getTexU(srcIndex);
            float v = src.lawrencium$getTexV(srcIndex);
