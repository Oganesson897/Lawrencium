...
public abstract class VertexBufferWriter implements VertexSink {
    ...
    protected VertexBufferWriter(VertexBufferView backingBuffer, BufferVertexType<?> vertexType) {
        ...
        this.vertexStride = this.vertexFormat.lawrencium$getStride();
    ...
    public void ensureCapacity(int count) {
        if (this.backingBuffer.lawrencium$ensureBufferCapacity((this.vertexCount + count) * this.vertexStride)) {
            this.onBufferStorageChanged();
        }
    }
    ...
    @Override
    public void flush() {
        this.backingBuffer.lawrencium$flush(this.vertexCount, this.vertexFormat);
        this.vertexCount = 0;
    }
