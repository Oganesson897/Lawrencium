...
public abstract class MixinBufferBuilder implements VertexBufferView, VertexDrain {
    ...
    @Unique
    private static int lawrencium$roundBufferSize(int amount) {
        int i = 2097152;
        if (amount == 0) {
            return i;
        } else {
    ...
    @Override
    public boolean lawrencium$ensureBufferCapacity(int bytes) {
        ...
        int newSize = this.byteBuffer.capacity() + lawrencium$roundBufferSize(bytes);
    ...
    @Override
    public ByteBuffer lawrencium$getDirectBuffer() {
        return this.byteBuffer;
    }

    @Override
    public int lawrencium$getWriterPosition() {
        return this.vertexCount * this.vertexFormat.getSize();
    }

    @Override
    public BufferVertexFormat lawrencium$getVertexFormat() {
        return BufferVertexFormat.from(this.vertexFormat);
    }

    @Override
    public void lawrencium$flush(int vertexCount, BufferVertexFormat format) {
    ...
    @Override
    public <T extends VertexSink> T lawrencium$createSink(VertexType<T> factory) {
        BlittableVertexType<T> blittable = factory.asBlittable();

        if (blittable != null && blittable.getBufferVertexFormat() == this.lawrencium$getVertexFormat())  {
